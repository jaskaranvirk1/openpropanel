...
- **Multi-user** — admin accounts manage the server; user accounts own their own domains, databases and files, each isolated under its own Linux system user. System users are provisioned **automatically** the first time an account needs one. Ownership is enforced on every action.
- **Deploy from GitHub** — paste a repo URL when creating a project (or link one later) and Open ProPanel clones it as the site's isolated system user, auto-detects the app (Laravel `public/`, SPA `dist/`, static, plain PHP), points the vhost at the right folder, and reloads. Pick a subfolder with a **Browse…** tree picker. Private repos get a generated read-only deploy key (one paste on GitHub). A per-repo **push-to-deploy webhook** (HMAC-signed) redeploys on every `git push`. Clones run in the background with live status on the project card. The panel deliberately does **not** run build steps — push built output, or build on the server yourself.
- **File manager** — a per-site file explorer jailed to the document root (kernel-enforced via `os.Root`): browse, edit, upload (multiple at once), download, rename, move, copy, chmod, and multi-select for bulk delete/move. **Compress to .zip** and **Extract** archives in place, with zip-slip containment and zip-bomb size caps. Panel-created files are handed to the site's system user automatically.
