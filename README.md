...
- **Deploy from GitHub** — paste a repo URL when creating a project (or link one later) and Open ProPanel clones it as the site's isolated system user, auto-detects the app (Laravel `public/`, SPA `dist/`, static, plain PHP), points the vhost at the right folder, and reloads. Private repos get a generated read-only deploy key (one paste on GitHub). A per-repo **push-to-deploy webhook** (HMAC-signed) redeploys on every `git push`. Clones run in the background with live status on the project card. The panel deliberately does **not** run build steps — push built output, or build on the server yourself.
